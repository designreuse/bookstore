...
export class AlarmdealComponent {
    ...
    images: any[];
    videoListShow: boolean = false;
    videoList: any[];
    videoItem: any;
    videoSrc = "";

    picListShow: boolean = false;
    picList: any[];
    picItem: any;
    url: string = environment.INTERFACE_URL;
    ...
    sendToasterMsg(msg) {
        this.showToast(msg.type, msg.title, msg.msg, "toast-bottom-right");
    }

    // 消息弹框
    showToast(type: string, title: string, msg: string, position: string) {
    ...
    dealalarmshow(alarminfo: AlarmData) {
        this.service.getAddress(alarminfo.ALARMINFO_LONGITUDE, alarminfo.ALARMINFO_LATITUDE)
            .then(result => {
    ...
    saveDealResult() {
        if (this.alarminfo != null) {
            ...
            if (this.dealPerson != null && this.dealPerson.replace(' ', '').length > 0) {
                ...
                this.alarminfo.ALARM_TREAT = null;
    ...
    //查看图片
    watchPic(alarminfo) {
        this.service.getMediaJSONForType(alarminfo.ID, "00").then(val => {
            this.images = [];
            val.forEach(element => {
                this.images.push({ source: element.shareAddress, alt: '', title: '' });
            });
            this.picShow = true;
        });
    }
    //观看视频
    watchVideo(alarminfo) {
        this.service.getMediaJSONForType(alarminfo.ID, "02").then(val => {
            this.videoList = val;
    ...
    //播放视频
    play(videoItem) {
        this.videoSrc = videoItem.shareAddress;
        this.videoShow = true;
    }

    //打开下载图片列表
    openPicList(alarminfo) {
        this.service.getMediaJSONForType(alarminfo.ID, "00").then(val => {
            this.picList = val;
            this.picListShow = true;
        });
    }

    //打开下载视频列表
    openVideoList(alarminfo) {
        this.service.getMediaJSONForType(alarminfo.ID, "02").then(val => {
            this.videoList = val;
            this.videoListShow = true;
    ...
    downloading() {
        this.showToast("success", null, "正在下载，请稍等！", "toast-top-center");
    }
    ...
    alarmdetailshow(alarminfo: AlarmData) {
        ...
        // this.service.getAlarmDataById(alarminfo).then(data => {
        //     this.alarminfo = data;
        //     this.platecolor = getPlateColorStr(alarminfo.PLATECOLOR);
    ...
    ngOnInit() {
        this.cols = [
            { field: 'CAR_AND_PLATECOLOR', header: '车牌', sort: false, style: { 'width': '12%', 'text-align': 'center' } },
            { field: 'superGroupName', header: '分组', sort: true, style: { 'width': '8%', 'text-align': 'center' } },
            { field: 'alarmName', header: '报警类型', sort: false, style: { 'width': '10%', 'text-align': 'center' } },
            { field: 'ALARM_LEVEL_STR', header: '报警等级', sort: true, style: { 'width': '10%', 'text-align': 'center' } },
            ...
            { field: 'ALARM_TREATTIME', header: '处理时间', sort: true, style: { 'text-align': 'center' } },
        ...
        this.images = [];
        this.images.push({ source: '', alt: '', title: '' });
