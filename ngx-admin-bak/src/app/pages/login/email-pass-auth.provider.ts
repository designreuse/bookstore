...
export class NbEmailPassAuthProvider extends  NbAbstractAuthProvider {
  ...
  protected defaultConfig: NgEmailPassAuthProviderConfig = {
    ...
    login: {
      ...
      defaultErrors: ['用户或密码填写不正确，请重试！'],
      defaultMessages: ['登录成功  正在跳转   请稍后...'],
    },
    register: {
      alwaysFail: false,
      rememberMe: true,
  ...
  authenticate(data?: any): Observable<NbAuthResult> {
    ...
    const url = environment.INTERFACE_URL + '/api/authenticate'; // this.getActionEndpoint('login');
    const aa = {'username': data.email, 'password': data.password, 'rememberMe': data.rememberMe};
    return this.http.request(method, url, { body: aa, observe: 'response' })
      .map((res) => {
