...
import { DataTable, Dropdown, OverlayPanel, SelectItem, Checkbox} from 'primeng/primeng';
...
import { ElectronicFenceService } from './electronic-fence.service';
import {
...
import { LbsTreeComponent } from '../lbstree/lbstree.component';
import { LbsMapComponent } from '../../components/map/map.component';
import { ElecFenceInfo } from './electronic-fence';

@Component({
    selector: 'ngx-electronic-fence',
    templateUrl: './electronic-fence.component.html',
    styleUrls: ['./electronic-fence.component.scss'],
    providers: [ ElectronicFenceService ],
})

export class ElectricFenceComponent implements OnInit, OnDestroy {
    constructor(private toasterService: ToasterService, private electronicFenceService: ElectronicFenceService,
    ...
    dialogDisplay2: boolean = false;
    dialogDisplay3:boolean = false;
    ...
    times: SelectItem[];
    //告警类型选项
    alarmTypes: SelectItem[];
    elecFence: ElecFenceInfo;
    elecFences: ElecFenceInfo[];
    selectedEleFence: ElecFenceInfo;
    cols: any[];


    cars: any = [
        {"fenceName":"测试围栏1","open":"开启","layerType":"范围","alarmType":"进区域告警","show":"显示"},
        {"fenceName":"测试围栏2","open":"开启","layerType":"范围","alarmType":"进区域告警","show":"不显示"},
    ]
    ...
    ngOnInit() {
        ...
        this.nodes = this.lbstree.getNodes();     //this.$sessionStorage.retrieve('carTreeData');
        ...
        // if(this.$sessionStorage.retrieve('searchCarNo') != null && this.$sessionStorage.retrieve('searchCarNo') != undefined && 
        // this.$sessionStorage.retrieve('searchCarNo') != '')
        // {
        ...
        // 地图初始化加载车辆定位
        // this.drawMovingMarker(false);

        //     this.leafletMap.on('baselayerchange', function (e) {
        //         miniMap.changeLayer(hongxinMini);
        //    })
        // //模板内容
        // const layerSwitchArray = document.getElementsByClassName('leaflet-control-layers-selector');
        // for (let i = 0; i < layerSwitchArray.length; i++) {
        //     const layerSwitch = layerSwitchArray.item(i);
        //     (layerSwitch as any).onclick = function () {
        //         // movingMarker.pause();
        //         alert()
        //     };
        // }
        ...
        //     const cumarker = L.marker([32.055761, 118.7981002],
        //         {title: 'car.CAR_NO', icon: myIcon2}).addTo(this.leafletMap);
        //     cumarker.bindPopup('<h1 hidden="hidden">,'+ 'lastTime' +',</h1><span>车牌号：</span>' +
        //        ' car.CAR_NO' + ' 车牌颜色：' )

        this.cols = [
            { field: 'fenceName', header: '电子围栏名称', sort: true },
            { field: 'open', header: '是否启用', sort: true },
            { field: 'layerType', header: '围栏类型', sort: true },
            { field: 'alarmType', header: '告警类型', sort: true },
            { field: 'show', header: '是否显示', sort: true },
        ];

        this.elecFence = new ElecFenceInfo();
        this.selectedEleFence = new ElecFenceInfo();
    }
        ...
        if (curMark !== null && curMark) {
            if (analyzeResult.commandId == 5122) {
                ...
                if (lastContent.indexOf('<p class="date1" hidden="hidden"/>') > 0) {
                    let warnInfo: string = '<p class="date1">告警类型：<b>' + exchangeWarnType(analyzeResult.warnTypeCode) + '</b></p>'
                        + '<p class="date1">告警时间：<b>' + analyzeResult.year + '-' + analyzeResult.month + '-' + analyzeResult.day + ' ' + analyzeResult.time + '</b></p>';
                    subLastContent = lastContent.replace('<p class="date1" hidden="hidden"/>', warnInfo);
                    (curMark as any).bindPopup(subLastContent);
                } else {
                    let content1: string = lastContent.substring(0, lastContent.indexOf('<p class="date1">告警类型：<b>'));
    ...
    getPopupContent(elecFence: ElecFenceInfo, event) {
        let layerType = event.layerType;
        let layer = event.layer;
        //todo 获取图层id，删除的时候根据id删除
        this.layer = layer;

        elecFence.layerType = layerType;
        // Marker/CircleMarker
        if (layerType == "marker" || layerType == "circlemarker") {
            elecFence.latlng = layer.getLatLng();
        // Circle
        } else if (layerType == "circle") {
            elecFence.latlng = layer.getLatLng();
            elecFence.radius = layer.getRadius();
        // Rectangle/Polygon
        } else if (layerType == "polygon" || layerType == "rectangle") {
            elecFence.latlng = layer.getLatLngs();
        // Polyline
    ...
    setLayer(event) {
        console.log(event);
        this.elecFence = new ElecFenceInfo();
        this.getPopupContent(this.elecFence,event);
        console.log(this.elecFence);
        this.drawed = true;
     ...
     */
    addLayerToMap(layer){
        this.map.addLayer(layer.layerType,layer.latlng,layer.radius);
    }
     ...
     */
    doSetElecFence() {
        this.electronicFenceService.setElecFenceInfo(this.elecFence)
        .subscribe(
            (val) => {
    ...
    doSearchElecFence() {
        ...
        this.electronicFenceService.doSearchElecFence()
        .subscribe(
            val => {
     ...
     */
    doEditElecFence() {
        this.dialogDisplay3 = true;
    }

    //Table行被选中时调用
    onRowSelect(event) {
        this.clearLayers();
        // this.elecFence = event.data;
        this.addLayerToMap(this.selectedEleFence);
    }
